import { FxError, LogProvider, M365TokenProvider, ManifestProperties, Result } from "@microsoft/teamsfx-api";
import { HubTypes } from "../../question/constants";
export declare class LaunchHelper {
    private readonly m365TokenProvider;
    private readonly logger?;
    constructor(m365TokenProvider: M365TokenProvider, logger?: LogProvider);
    getLaunchUrl(hub: HubTypes, teamsAppId: string, properties: ManifestProperties, withLoginHint?: boolean): Promise<Result<string, FxError>>;
    getM365AppId(teamsAppId: string): Promise<Result<string, FxError>>;
    private getTidFromToken;
    private getUpnFromToken;
}
//# sourceMappingURL=launchHelper.d.ts.map